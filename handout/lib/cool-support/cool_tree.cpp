...
{
    return new method_class(name, formals->copy_list(), return_type_decl, expr->copy_Expression());
}
...
void method_class::dump(std::ostream &stream, int n)
{
    ...
    dump_Symbol(stream, n + 2, return_type_decl);
...
{
    return new dispatch_class(expr->copy_Expression(), name,
                              actual->copy_list());
}
