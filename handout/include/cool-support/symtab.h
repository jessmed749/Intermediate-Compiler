...
class SymbolTable
{
  ...
  public:
    ...
    void insert(Symbol const &k, V *v)
    {
    ...
    V *find(Symbol const &k) const
    {
        assert(!this->scopes.empty() && "probe: No scope in symbol table.");
    ...
    V *find_in_scopes(Symbol const &k) const
    {
...
template<typename V>
std::ostream &operator<<(std::ostream &s, SymbolTable<V> const &symtab)
{
    symtab.dump(s);
    return s;
}
